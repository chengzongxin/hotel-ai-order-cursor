...
def get_required_order_fields(service_type: str | None, order_info: dict[str, object]) -> list[str]:
    ...
    if service_type == SERVICE_TYPE_SINGLE_REPAIR:
        return ["product", "fault", "expected_start_time"]
    if service_type == SERVICE_TYPE_SINGLE_INSTALL:
        return ["product", "expected_start_time", "goods_arrival_status"]
    if service_type == SERVICE_TYPE_SINGLE_MEASURE:
...
def normalize_order_card_update(
    ...
    for key, raw_value in updates.items():
        ...
        if key == "area_room":
            if service_type == SERVICE_TYPE_MANAGED_REPAIR and is_public_area_text(value):
                normalized["area"] = "公区"
