...
sequenceDiagram
    ...
    participant RAG as 商品检索
    participant EXT as 用户端/App 接口
    participant CK as SQLite Checkpoint

    U->>FE: 说出下单需求
    FE->>API: POST /api/chat/stream
    API->>G: graph.astream(messages, session_id, user)
    G->>CK: 恢复会话状态
    G->>LLM: intent_node 识别意图并抽取字段
    G->>RAG: search_product_node 检索 Top3 商品
    G-->>FE: preview: phase=product_selection
    U->>FE: 点选商品
    FE->>API: POST /api/chat/{session_id}/select-product
    API->>EXT: 托管维修维保范围/上下文查询
    API->>CK: 更新 selected_product_code、order_card_fields
    API-->>FE: order_preview
    U->>FE: 编辑联系人/电话/时间
    FE->>API: PATCH /api/chat/{session_id}/order-info
    API->>CK: 归一化并更新 order_info
    U->>FE: 点击确认下单
    FE->>API: POST /api/chat/{session_id}/confirm
    API->>EXT: 构造参数并调用真实下单接口
    API->>CK: 写入 submission/submitted_order/last_order
    API-->>FE: 成功或失败的 order_preview
```

这条时序里有一个核心原则：**自然语言走 LangGraph + LLM，明确 UI 操作走确定性接口**。商品选择、字段编辑、按钮确认都已经是结构化事件，不再交给 LLM 猜。

## 状态机与节点编排

主流程由 `workflow/builder.py` 中的 LangGraph 状态机驱动。状态结构定义在 `workflow/state.py`，每个节点返回一个字典来更新 `AgentState`。
...
| 路由点 | 条件 | 下一节点 |
| --- | --- | --- |
| `route_after_intent` | `cancel_order` 或 `order_info.user_cancelled=true` | `cancel_node` |
| `route_after_intent` | 有候选商品且用户输入 `1/2/3/以上都不符合` | `search_product_node` |
| `route_after_intent` | `create_order` / `confirm_order` | `search_product_node` |
| `route_after_intent` | `smalltalk` / `unknown` 且无活跃订单 | `assist_node` |
| `route_after_intent` | 其他情况，尤其是有活跃订单时偏题 | `ask_node` |
| `route_after_search_product` | 用户选择"以上都不符合" | `ask_node` |
| `route_after_search_product` | 有候选商品但还未选中 | `ask_node` |
| `route_after_search_product` | 已选中商品或无商品可继续校验 | `coverage_node` |
| `route_after_validation` | `missing_info` 非空 | `ask_node` |
| `route_after_validation` | 必填信息完整 | `confirm_node` |
| `route_after_confirm` | `order_info.user_confirmed=true` | `submit_node` |
| `route_after_confirm` | 等待用户确认 | `END` |
...
| `prepare_order_context_node` | 读取维保卡、用户资料、地址、全局配置等默认值，并由 `workflow/order_fields.py` 生成前端卡片字段。 | 输出 `order_context`、`order_card_fields`、`phase=pre_order` |
...
| `submit_node` | 调用 `workflow/submission.py::submit_order_from_state`，构造真实参数并提交订单。成功后写入 `submitted_order/last_order` 并清空活跃订单状态。 | 输出 `submission`、`phase=submitted/pre_order` |
...
| --- | --- | --- |
| `idle` | 没有活跃订单 | 普通聊天空状态 |
| `collecting` | 已识别下单意图，但商品或字段还不足 | 追问文本、可能无卡片 |
| `product_selection` | 已检索到候选商品，但尚未选中 | `ProductSelectionCard` |
| `pre_order` | 已选商品并生成预下单字段 | `OrderPreviewCard`、缺字段/维保提示 |
| `submitted` | 真实提交成功 | `OrderSuccessCard` |
| `cancelled` | 当前订单已取消 | 不再展示活跃订单卡片 |
...
| 场景 | 代码位置 | 说明 |
| --- | --- | --- |
| 意图识别和字段抽取 | `intent_node` + `prompts/intent/intent.md` | 使用结构化输出 `IntentResult`，只产出 JSON 字段 |
| 缺字段追问 | `ask_node` + `prompts/ask/missing_info.md` | 一次追问最关键字段，部分字段使用固定兜底文案 |
| 偏题纠偏 | `ask_node` + `prompts/ask/off_topic.md` | 有活跃订单时礼貌拉回当前下单任务 |
| 闲聊/简单问答 | `assist_node` + `workflow/agent.py` | 无活跃订单时才进入辅助 Agent |
| 确认/取消/提交话术 | `confirm_node` / `cancel_node` / `submit_node` | 多数为模板文本，避免把确定结果再交给模型生成 |
...
| `selected_product_code` | 当前选中的商品编码；未选中时为空，由用户点选或输入序号后写入 |
...
| `api/routes.py` | `/api/chat`、流式对话、商品选择、字段更新、确认提交、历史查询、清空会话接口 |
| `workflow/state.py` | LangGraph `AgentState` 定义 |
| `workflow/builder.py` | LangGraph 节点编排、状态机装配、对话运行入口 |
| `workflow/confirmation_policy.py` | 预下单确认话术、商品匹配反馈、维保范围外提示拼装 |
| `workflow/coverage_policy.py` | 维保范围校验前置判断、非托管服务跳过校验、校验结果状态转换 |
| `workflow/intent_policy.py` | LLM 意图抽取结果到 `order_info` / `phase` 的合并策略 |
| `workflow/messages.py` | LangChain 消息历史格式化、最近用户消息/AI 消息提取 |
| `workflow/routes.py` | LangGraph conditional edge 路由函数 |
| `workflow/preview.py` | 从 LangGraph state 构造前端 `order_preview` JSON |
| `workflow/questions.py` | 缺字段追问、偏题纠偏回复等问答策略 |
| `workflow/session_access.py` | 会话 checkpoint 所属用户校验 |
| `workflow/session_actions.py` | 前端点选商品、编辑预下单卡片、按钮确认提交等确定性会话操作 |
| `workflow/order_context_loader.py` | 下单上下文读取及失败兜底 |
| `workflow/order_defaults.py` | LLM 抽取字段后的默认值填充、托管维修客房/公区归一化、时间合并 |
| `workflow/order_validation_policy.py` | 预下单卡片字段准备、必填字段校验、重试次数更新 |
| `workflow/order_fields.py` | 兼容旧 import 的订单字段策略 re-export |
| `services/order_policy.py` | 必填字段、预下单卡片字段、前端编辑字段归一化等领域规则 |
| `services/service_types.py` | 服务类型常量、维修类判断、真实提交路由映射 |
| `workflow/submission.py` | 订单提交和提交成功后的状态清理 |
| `workflow/agent.py` | 基于 `create_agent` 的辅助 Agent |
| `workflow/llm.py` | 主 LLM 初始化、`get_llm_run_config()` 追踪配置合并 |
| `workflow/llm_callbacks.py` | LLM 调用参数与回复的本地日志 callback |
| `workflow/middleware.py` | 辅助 Agent 的模型/工具日志、重试、调用次数限制 |
| `workflow/studio.py` | LangGraph Studio 入口 |
| `prompts/` | 文件化 Prompt |
| `repositories/product_store.py` | Chroma 向量库封装，商品检索主路径 |
| `repositories/qwen_embedding.py` | Qwen text-embedding 客户端 |
| `repositories/spu_loader.py` | Excel SPU 数据加载和服务类型归一化 |
...
- `thread_id` 由 `user_id + session_id` 组合生成，同一个 `session_id` 在不同用户下不会串状态。
- 读取或更新已有会话时会调用 `ensure_session_access()`，发现 checkpoint 中的 `user_id` 与当前用户不一致会返回 403。

## Agent Middleware

主下单流程仍然由手写 LangGraph 状态机控制，保证订单生命周期可预测。项目另外通过 `workflow/agent.py` 接入 LangChain 官方 `create_agent()`，并通过 `workflow/middleware.py` 增加日志、重试和调用次数限制，用于处理无活跃订单时的闲聊、未知问题和简单工具问答。
...
项目通过 `POST /api/chat/stream` 提供 NDJSON 流式响应，前端可以边处理边展示状态。后端使用 LangGraph 官方 `workflow.astream(..., stream_mode=["updates", "messages", "custom"], version="v2")`。
...
**向量库构建**（`repositories/product_store.py`）：
...
调试错配时可在 `search_product_tool` 传入 `include_diagnostics=true`，返回 `diagnostics`：

- `vector_score`：Chroma 原始相似度。
- `keyword_overlap`：商品名是否和 query 有关键词重叠。
- `penalty` / `adjusted_score`：故障惩罚和最终排序分。
- `filtered_reason`：被过滤原因，如 `keyword_mismatch`、`below_threshold`、`outside_top_k`。
- `fallback_to_vector_results`：关键词过滤过严时是否回退到纯向量结果。

HTTP 响应中的诊断字段由 `schemas/product.py::ProductSearchDiagnostics` 约束，候选明细由 `ProductSearchDiagnosticCandidate` 约束，避免调试字段继续漂移。

HTTP 调试接口 `/api/products/search` 也支持相同能力：
...
提交失败不会清空活跃订单字段，用户可以继续编辑卡片后再次确认。只有 `submission.state=succeeded` 时才进入 `submitted`，并清空 `order_info/products/selected_product_code` 等活跃状态。
...
- `workflow/llm_callbacks.py`：`LLMCallTraceHandler`，监听 LangChain callback。
- `workflow/llm.py`：`get_llm_run_config()`，在每次 `invoke/astream` 时注入 callback。
...
- `search_product_node` 的商品向量检索（走 `repositories/qwen_embedding.py` 的 `/embeddings`，不是 Chat LLM）
...
前端位于 `frontend/`，主页面是 `frontend/src/App.vue`，接口调用和状态派生主要拆到 composable 中。展示组件已经拆分到：

| 路径 | 职责 |
| --- | --- |
| `frontend/src/App.vue` | 页面组装、会话状态接线、商品选择、字段更新、确认/取消等事件处理 |
| `frontend/src/composables/useChatApi.ts` | `/api/chat/stream`、fallback、商品选择、字段更新、确认提交接口调用 |
| `frontend/src/composables/useChatSession.ts` | `session_id`、消息列表、本地历史、滚动与消息更新 |
| `frontend/src/composables/useOrderPreview.ts` | 从 `order_preview` 派生前端显示状态和按钮可用状态 |
| `frontend/src/types/order.ts` | 前端使用的 `OrderPreview`、商品、卡片字段等类型 |
| `frontend/src/types/product.ts` | 商品库和商品检索诊断类型 |
| `frontend/src/views/ProductTest.vue` | 商品库浏览、商品检索和检索诊断调试页 |
...
- `selected_product_code` 不默认等于 Top1；有候选商品但未选中时应停在 `product_selection`，由用户明确选择。
- `order_submit_route` 只由 `services/service_types.py::resolve_order_submit_route()` 映射得到：`managed_repair`、`single_repair`、`single_install`、`single_measure`。
- 修改订单字段时，要同步 `workflow/order_fields.py`、`schemas/order_preview.py`、`frontend/src/types/order.ts`。
- 订单必填字段、预下单字段、前端字段回写规则优先改 `services/order_policy.py`；`workflow/order_fields.py` 只保留兼容导出。
- 修改缺字段追问或偏题回应时，优先改 `workflow/questions.py` 与 `prompts/ask/`。
- 修改消息历史提取逻辑时，优先改 `workflow/messages.py`，不要把消息扫描逻辑散落到节点里。
- 修改状态字段时，要同步 `workflow/state.py`、`build_order_preview_model()` 和前端 computed。
...
- 新增节点时要同时补 `NODE_STATUS_MESSAGES`，否则流式前端只能展示默认"正在处理您的请求..."。
- 新增可编辑卡片字段时要补 `normalize_order_card_update()`，否则前端看似改了，真实提交仍可能用旧值。
...
├── core/                        # 配置、数据库、日志
├── services/                    # 订单、服务类型等业务规则
├── docs/                        # 设计文档、测试用例、追踪说明
├── frontend/                    # Vue 3 前端
├── workflow/                    # LangGraph 状态机、节点、路由和 Studio 入口
├── memory/                      # PostgreSQL 日志与旧 memory 模块
├── prompts/                     # 文件化 Prompt（见 docs/prompts.md）
├── repositories/                # 商品库、Embedding、Excel SPU 数据访问
├── schemas/                     # API 请求/响应模型
├── tests/                       # 集成测试（pytest + pytest-asyncio）
├── tools/                       # LangChain Tool、真实下单和外部接口工具
├── .env.example                 # 环境变量示例
├── docker-compose.yml
...
2. 主状态字段使用 `intent`、`phase`、`service_type`、`effective_service_type`、`order_info`、`missing_info`。
...
6. 商品检索主路径是 `repositories/product_store.py`（`ProductVectorStore` / Chroma）。工具层入口是 `tools/product_search.py::search_product_tool`，节点是 `search_product_node`，HTTP 接口 `/api/products/search` 也统一走这个工具；检索元数据（`status` / `query` / `feedback`）在 API 层由 `products` 推导，不再单独写入 state。
7. Excel 原始字段 `service_product_code`、`service_product_name`、`service_order_type` 是业务数据列名，可以保留。
8. 修改状态字段时，需要同步更新：
   - `workflow/state.py`
   - `workflow/builder.py`
   - `workflow/order_fields.py`
   - `workflow/submission.py`
   - `prompts/`
...
- 可执行策略评测：`tests/fixtures/workflow_policy_cases.json` 用 JSON 固化订单策略、预览和路由期望，不调用 LLM 或真实接口。
- Fake LLM 多轮评测：`tests/fixtures/fake_llm_workflow_cases.json` 使用 fake structured LLM 和 fake 工具跑真实 `run_agent()`，验证 checkpoint、多轮合并、商品选择、取消等主流程。
...
GitHub Actions 中也配置了同一组护栏：`.github/workflows/guardrails.yml` 会在 PR 和 `main` 推送时运行后端 guardrails，并使用 Node 24 构建前端。
